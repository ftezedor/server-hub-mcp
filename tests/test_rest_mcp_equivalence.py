...
def test_create_alert_rest_then_mcp(api: httpx.Client) -> None:
    server = _choose_server(api)
    marker = f"equivalence-rest-to-mcp-{uuid.uuid4().hex}"
    ...
    matches = [
        alert
        for alert in mcp_payload["alerts"]
        if alert.get("message") == marker
    ]

    assert len(matches) == 1
    alert = matches[0]
    assert alert["server"]["name"] == server["name"]
    assert alert["server"]["ip"] == server["ip"]
    assert alert["severity"] == "warning"
    assert alert["created_at"] is not None


def test_create_alert_mcp_then_rest(api: httpx.Client) -> None:
    server = _choose_server(api)
    marker = f"equivalence-mcp-to-rest-{uuid.uuid4().hex}"
    ...
    assert mcp_payload["created"] is True
    assert mcp_payload["server"]["name"] == server["name"]
    assert mcp_payload["server"]["ip"] == server["ip"]
    assert mcp_payload["severity"] == "warning"
    assert mcp_payload["message"] == marker
    assert mcp_payload["created_at"] is not None

    rest = api.get("/alerts")
    rest.raise_for_status()
    matches = [
        alert
        for alert in rest.json()["alerts"]
        if alert.get("message") == marker
    ]

    assert len(matches) == 1
    alert = matches[0]
    assert alert["server"] == server["name"]
    assert alert["severity"] == "warning"
    assert alert["created_at"] is not None
