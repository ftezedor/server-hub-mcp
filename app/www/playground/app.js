...
function updateExecutionSummary(toolCount, llmTurns, startedAt) {
  const panel = $("executionSummary");
  if (!panel) return;

  const toolCountEl = $("summaryToolCount");
  const llmTurnsEl = $("summaryLlmTurns");
  const operationsEl = $("summaryOperations");
  const durationEl = $("summaryDuration");

  if (toolCountEl) toolCountEl.textContent = String(toolCount);
  if (llmTurnsEl) llmTurnsEl.textContent = String(llmTurns);
  if (operationsEl) operationsEl.textContent = String(toolCount);
  if (durationEl) durationEl.textContent = `${Date.now() - startedAt} ms`;
...
function renderMarkdown(markdown) {
  ...
  const flushTable = () => {
    ...
    const rows = table.map(line => line.split("|").slice(1, -1).map(cell => cell.trim())).filter(row => row.length);
    if (rows.length < 2 || !rows[1].every(cell => /^:?-{3,}:?$/.test(cell))) { paragraph.push(...table); table = []; return; }
    let out = "<table><thead><tr>";
    rows[0].forEach(cell => out += `<th>${inlineMarkdown(cell)}</th>`);
    out += "</tr></thead><tbody>";
    rows.slice(2).forEach(row => { out += "<tr>"; for (let i=0;i<rows[0].length;i++) out += `<td>${inlineMarkdown(row[i] ?? "")}</td>`; out += "</tr>"; });
    html.push(out + "</tbody></table>"); table = [];
  };
  for (const line of lines) {
    if (line.trim().startsWith("|") && line.trim().endsWith("|")) { flushParagraph(); table.push(line.trim()); continue; }
    flushTable();
    if (!line.trim()) { flushParagraph(); continue; }
    if (/^#{1,3}\s/.test(line)) { flushParagraph(); html.push(`<strong>${inlineMarkdown(line.replace(/^#{1,3}\s+/, ""))}</strong>`); continue; }
    paragraph.push(line);
  }
...
function renderActivity(toolName, argumentsValue, result) {
  ...
  if (activity.querySelector("p")) activity.innerHTML = "";
  ...
  item.append(title, args, resultTitle, resultElement);
  ...
  try {
    ...
    const response = await fetch(`${url}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: "system", content: "Answer directly and concisely. Do not include internal analysis." },
          { role: "user", content: "Introduce yourself briefly by saying: I am [your model name]." }
        ]
      })
    });
...
$("ask").addEventListener("click", async () => {
  ...
  try {
    ...
    let executionToolCount = 0;
    let executionLlmTurns = 0;
    ...
    while (round < maxToolRounds) {
      ...
      // Keep MCP tools available on every LLM turn so the model can
      // iteratively request additional tools after receiving a result.
      const llmStartedAt = Date.now();
      const assistantMessage = await callLlm(messages, mcpTools);
      ...
      if (!toolCalls.length) {
        addCommunicationStep(
          `LLM response received · ${llmDuration} ms`,
          "done"
        );

        $("answerPanel").classList.remove("hidden");
        answer.innerHTML = renderMarkdown(
          assistantMessage.content || "The model returned no final answer."
        );

        if (typeof updateExecutionSummary === "function") {
      ...
      for (const toolCall of toolCalls) {
        ...
        renderActivity(toolName, argumentsValue, toolResult);
